...
attach(MetaRareFun)
FunRegOT<-ggplot(MetaRareFun,aes(x=RichnessThawed, y=RichnessOriginal))+
  geom_point(size=4, aes(col=Treatment),show.legend=F)+ 
  ...
  labs(x = "Thawed Species Richness", y="Original Species Richness")+
  annotate("text", x=365,  y=600,label ="Pearsons R = 0.89; p < 0.0001", size=4)+
  labs(title = "A", size= 45);FunRegOT


FunRegOF<-ggplot(MetaRareFun,aes(x=RichnessFrozen, y=RichnessOriginal))+
  geom_point(size=4, aes(col=Treatment),show.legend=F)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  theme_bw()+ 
  theme(legend.key.size = unit(5, "mm"),
  ...
  ylim(0,600)+
  #scale_y_continuous(breaks=seq(0,600,by=100))+
  labs(x = "Frozen Species Richness",y="Original Species Richness")+
  annotate("text", x=200,  y=600,label ="Pearsons R = 0.55; p = 0.01", size=4)+
  labs(title = "B", size= 45);FunRegOF


FunRegFT<-ggplot(MetaRareFun,aes(x=RichnessThawed, y=RichnessFrozen))+
  geom_point(size=4, aes(col=Treatment),show.legend=T)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  theme_bw()+ 
  theme(legend.key.size = unit(5, "mm"),
        panel.grid = element_blank(),
  ...
  annotate("text", x=375, y=600,label ="Pearsons R = 0.66; p = 0.001", size=4)+
  labs(title = "C", face="bold")+
  guides(colour = guide_legend(nrow = 1));FunRegFT



FunAlpha<-FunRegOT + FunRegOF + FunRegFT  +  plot_layout(guides = 'collect') &
  theme(legend.position = 'bottom');FunAlpha


pdf("Fungi/Analysis/Diversity/Graphs/Alpha-Cor-Fun-panels-Trt.pdf", height=5.5, width=12,onefile=FALSE)
FunAlpha
dev.off()
...
#***************************************************************************************************************----------
attach(MetaRareBac)
BacRegOT<-ggplot(MetaRareBac,aes(x=RichnessThawed, y=RichnessOriginal))+
  geom_point(size=4, aes(col=Treatment),show.legend=F)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  theme_bw()+ 
  theme(legend.key.size = unit(5, "mm"),
  ...
  labs(x = "Thawed Species Richness",y="Original Species Richness")+
  annotate("text", x=600,  y=-100,label ="Pearsons R = 0.62; p = 0.005", size=4)+
  labs(title = "A", size= 45);BacRegOT

#Automatically removes the missing sample
BacRegOF<-ggplot(MetaRareBac,aes(x=RichnessFrozen, y=RichnessOriginal))+
  geom_point(size=4, aes(col=Treatment),show.legend=F)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  theme_bw()+ 
  theme(legend.key.size = unit(5, "mm"),
  ...
  #scale_x_continuous(breaks=seq(0,1000,by=200))+
  labs(x = "Frozen Species Richness",y="Original Species Richness")+
  annotate("text", x=600, y=-100,label ="Pearsons R = 0.63; p = 0.004", size=4)+
  labs(title ="B", size= 45);BacRegOF


BacRegFT<-ggplot(MetaRareBac,aes(x=RichnessThawed, y=RichnessFrozen))+
  geom_point(size=4, aes(col=Treatment),show.legend=T)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  theme_bw()+ 
  theme(legend.key.size = unit(5, "mm"),
  ...
  labs(title = "C", size= 45)+
  guides(colour = guide_legend(nrow = 1 ));BacRegFT




BacAlpha<-BacRegOT + BacRegOF + BacRegFT +  plot_layout(guides = 'collect') &
  theme(legend.position = 'bottom');BacAlpha
        
        

pdf("Bacteria/Analysis/Diversity/Graphs/Alpha-Cor-Bac-panels-Trt.pdf", height=5.5, width=12,onefile=FALSE)
BacAlpha
dev.off()
...
BacOTCor<-cor.test(MetaRareBac$RichnessOriginal,MetaRareBac$RichnessThawed, method="pearson");BacOTCor#cor 0.62;p=0.004814
...
FunOTCor<-cor.test(MetaRareFun$RichnessOriginal,MetaRareFun$RichnessThawed, method="pearson");FunOTCor#cor=0.89;p=1.186e-07
...
MetaRareF<-read.csv("Fungi/Metadata/MetaRareFun-CNF9WT2.csv")
MetaRareB<-read.csv("Bacteria/Metadata/MetaRareBac-CNF9WT2.csv")
...
#Import metadata .....................................................................................................
MetaFO<-read.csv("Fungi/Metadata/Correlations/MetaRareFun-Original.csv", row.names=1);dim(MetaFO)#19x14
MetaFT<-read.csv("Fungi/Metadata/Correlations/MetaRareFun-Thawed.csv", row.names=1);dim(MetaFT)#19x14
MetaFF<-read.csv("Fungi/Metadata/Correlations/MetaRareFun-Frozen.csv", row.names=1);dim(MetaFF)#19x14

MetaBO<-read.csv("Bacteria/Metadata/Correlations/MetaRareBac-Original.csv", row.names=1);dim(MetaBO)#19x15
MetaBT<-read.csv("Bacteria/Metadata/Correlations/MetaRareBac-Thawed.csv", row.names=1);dim(MetaBT)#19x15
MetaBF<-read.csv("Bacteria/Metadata/Correlations/MetaRareBac-Frozen.csv", row.names=1);dim(MetaBF)#19x15



#Import asv tables..................................................................................................
OtuTrans2FO<-read.csv("Fungi/ASVtables/Correlations/OtuTrans2Cor-Orig.csv",row.names = 1); dim(OtuTrans2FO)#19x3193
OtuTrans2FT<-read.csv("Fungi/ASVtables/Correlations/OtuTrans2Cor-Thawed.csv",row.names = 1);dim(OtuTrans2FT)#19x3193
OtuTrans2FF<-read.csv("Fungi/ASVtables/Correlations/OtuTrans2Cor-Frozen.csv",row.names = 1);dim(OtuTrans2FF)#19x3193


OtuTrans2BO<-read.csv("Bacteria/ASVtables/Correlations/OtuTrans2Cor-Orig.csv",row.names = 1);dim(OtuTrans2BO)#19x9429
OtuTrans2BT<-read.csv("Bacteria/ASVtables/Correlations/OtuTrans2Cor-Thawed.csv",row.names = 1);dim(OtuTrans2BT)#19x9429
OtuTrans2BF<-read.csv("Bacteria/ASVtables/Correlations/OtuTrans2Cor-Frozen.csv",row.names = 1);dim(OtuTrans2BF)#19x9429
...
names(FunOTdata) <- c("SampleID","Treatment","Original","Thawed");head(FunOTdata[1:3])
names(FunOFdata) <- c("SampleID","Treatment","Original","Frozen");head(FunOFdata[1:3])
names(FunTFdata) <- c("SampleID","Treatment","Thawed","Frozen");head(FunTFdata[1:3])

names(BacOTdata) <- c("SampleID","Treatment","Original","Thawed");head(BacOTdata[1:3])
names(BacOFdata) <- c("SampleID","Treatment","Original","Frozen");head(BacOFdata[1:3])
names(BacTFdata) <- c("SampleID","Treatment","Thawed","Frozen");head(BacTFdata[1:3])
...
CorFOT<-ggplot(FunOTdata,aes(x=Thawed, y=Original))+
  geom_point(size=2.5, aes(col=Treatment),show.legend=F)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  scale_colour_manual(values = c("#664027","#45877f"))+ 
  theme_bw()+ 
  ...
  annotate("text", x=0.85,  y=0.4,label ="Mantel R = 0.87; p < 0.0001", size=3.5)+
...
CorFOF<-ggplot(FunOFdata,aes(x=Frozen, y=Original))+
  ...
  annotate("text", x=0.85,  y=0.4,label ="Mantel R = 0.73; p < 0.0001", size=3.5)+
  labs(title = "B", size= 45);CorFOF


CorFTF<-ggplot(FunTFdata,aes(x=Thawed, y=Frozen))+
  geom_point(size=2.5, aes(col=Treatment),show.legend=T)+ 
  geom_smooth(method=lm, se=TRUE, alpha=0.2, col="black")+
  scale_colour_manual(values = c("#664027","#45877f"))+ 
  theme_bw()+ 
  theme(panel.grid = element_blank(),
        ...
        legend.position = "bottom")+ 
  xlim(0.4,1.0) +ylim(0.4,1.0)+
  labs(x = "Thawed Beta Diversity", y="Frozen Beta Diversity") +
  annotate("text", x=0.85,  y=0.4,label ="Mantel R = 0.69; p < 0.0001", size=3.5)+
  labs(title = "C", size= 45)+guides(colour = guide_legend(nrow = 1));CorFTF
...
CorBTF<-ggplot(BacTFdata,aes(x=Thawed, y=Frozen))+
  ...
  labs(title = "C", size= 45)+ guides(colour = guide_legend(nrow = 1));CorBTF
...
BacReg<-CorBOT+ CorBOF + CorBTF +  plot_layout(guides = 'collect') &
  theme(legend.position = 'bottom');BacReg


FunReg<- CorFOT+ CorFOF + CorFTF  +  plot_layout(guides = 'collect') &
  theme(legend.position = 'bottom');FunReg




#Fungal plots.................................................................................................
pdf("Fungi/Analysis/Diversity/Graphs/Beta-Reg-Fun-Panels.pdf", height=6, width=12,onefile=FALSE)
FunReg
dev.off()
...
#Bacterial plots.................................................................................................
pdf("Bacteria/Analysis/Diversity/Graphs/Beta-Reg-Bac-Panels.pdf", height=6, width=12,onefile=FALSE)
BacReg
dev.off()
